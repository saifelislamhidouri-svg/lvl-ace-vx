import requests ,os ,sys ,jwt ,pickle ,json ,binascii ,time ,urllib3 ,base64 ,datetime ,re ,socket ,threading ,random
...
def AuToUpDaTE():
    ...
    # تجنب فشل طلب JSON
    try:
        r = requests.get('http://217.160.125.15:3000/up', timeout=5)
        if r.status_code == 200:
            data = r.json()
            url = data.get('sErVeR', 'https://clientbp.ggblueshark.com')
            ob = data.get('ob', 'OB52')
        else:
